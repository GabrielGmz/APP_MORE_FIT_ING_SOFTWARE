<template>
  <div class="container">
      ...
      <div v-else class="account-card">
        ...
        <form @submit.prevent="guardarCambios" class="account-form">
          ...
          <div class="form-actions">
            <button type="button" class="btn-cancel" @click="$router.push('/Configuracion_C')" :disabled="guardando">
              CANCELAR
            </button>
    ...
    <nav class="bottom-nav">
      ...
      <!-- Icono Activo (usando el de configuración ya que es subsección) -->
...
const mensaje = ref('')
const tipoMensaje = ref('success') // 'success' or 'error'

const cargarUsuario = async (user) => {
  try {
    ...
    console.error(error)
    mensaje.value = 'Error al cargar la información.'
...
const guardarCambios = async () => {
  ...
  try {
    ...
    // 2. Actualizar contraseña en Auth si se proporcionó
    if (contrasena.value) {
    ...
    if (error.code === 'auth/requires-recent-login') {
      mensaje.value = 'Por seguridad, debes volver a iniciar sesión para cambiar tu contraseña.'
    } else {
      mensaje.value = 'Error al guardar los cambios. Intenta de nuevo.'
    }
    tipoMensaje.value = 'error'
  } finally {
