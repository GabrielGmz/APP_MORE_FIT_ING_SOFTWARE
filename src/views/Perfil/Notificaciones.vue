...
onMounted(async () => {
  ...
  usuarios.value = querySnapshot.docs
    ...
    .filter((u) => {
      // Consider both "rol" and "role" fields; include users with role "entrenador" or with no role defined
      const roleField = (u.rol ?? u.role ?? "").toString().trim().toLowerCase();
      return roleField === "entrenador" || roleField === "";
    });
});
