...
const guardarPlanModal = async () => {
  ...
  if (!planForm.value.titulo.trim()) return alert("Ingrese un título")
  try {
    if (planEditando.value) {
      await updateDoc(
        doc(db, "usuarios", clienteSeleccionado.value, "planesNutricion", planEditando.value.id),
        { ...planForm.value }
      )
    } else {
      await addDoc(
        collection(db, "usuarios", clienteSeleccionado.value, "planesNutricion"),
        { ...planForm.value, fecha: new Date() }
      )
    }
    cerrarModal()
    await cargarPlanes()
