<template>
  <div class="container">
    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
        viewBox="0 0 42 42" @click="$router.push('/P_P_Entrenador')" style="position: absolute; top: 35px; left: 15px; cursor: pointer;">
    ...
    <div v-else>
      ...
      <ul v-else>
        <li v-for="rutina in rutinas" :key="rutina.id">
          <h3>{{ rutina.titulo }}</h3>
          <p><strong>Instrucciones:</strong> {{ rutina.instrucciones }}</p>
          <p><strong>Repeticiones:</strong> {{ rutina.repeticiones }}</p>
...
const guardarRutinaModal = async () => {
  ...
  try {
    if (rutinaEditando.value) {
      await updateDoc(
        doc(db, "usuarios", clienteSeleccionado.value, "rutinas", rutinaEditando.value.id),
        { ...rutinaForm.value }
      )
    } else {
      await addDoc(
        collection(db, "usuarios", clienteSeleccionado.value, "rutinas"),
        { ...rutinaForm.value, fecha: new Date() }
      )
    }
    cerrarModal()
    await cargarRutinas()
...
.loading {
  width: 100%;
  max-width: 100px;
  height: 8px;           /* alto de la barrita */
  background: #9dfff2;   /* color de fondo */
  overflow: hidden;
  position: relative;
  border-radius: 10px;
  margin: 20px auto;     /* centra la barrita */
