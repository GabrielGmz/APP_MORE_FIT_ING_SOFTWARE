...
async function enviarMembresia() {
  try {
    ...
    const membresiaRef = collection(db, "membresias"); // ✅ Colección global
...
async function cargarMembresias() {
  const membresiaRef = collection(db, "membresias"); // ✅ ahora global
...
onMounted(() => {
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) cargarMembresias(user.uid);
  });
});
...
async function borrarMembresia(index) {
  try {
    ...
    await deleteDoc(doc(db, "usuarios", uid, "membresias", id));
