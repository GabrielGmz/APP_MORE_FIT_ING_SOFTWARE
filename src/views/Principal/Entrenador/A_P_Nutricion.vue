<template>
  <div class="container">
    ...
    <div v-else>
      ...
      <ul v-else class="plan_c">
        <li v-for="plan in planes" :key="plan.id" class="plan_item">
          <h3>{{ plan.titulo }}</h3>
          <p><strong>Instrucciones:</strong> {{ plan.instrucciones }}</p>
          <p><strong>Comidas:</strong> {{ plan.comidas }}</p>
          <button @click="editarPlan(plan)" class="btn_editarp">Editar</button>
          <button @click="eliminarPlan(plan.id)" class="btn_eliminar">Eliminar</button>
        </li>
      </ul>
    </div>
